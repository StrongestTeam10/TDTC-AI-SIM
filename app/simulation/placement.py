...
def _random_point_in_polygon(
    ...
    후보는 축 정렬 경계 상자(poly.bounds)가 아니라 최소 회전 사각형
    (minimum_rotated_rectangle) 안에서 뽑는다. 대각선으로 길게 뻗은 좁은
    폴리곤(실제 시장 골목 모양)은 축 정렬 경계 상자가 실제 면적보다 훨씬
    커서 거부율이 매우 높아진다.
...
def place_visitors(
    ...
    centerline_strength 기본값 0.18은 통로 중앙 선호 경향은 유지하면서도
    좌우로 자연스럽게 퍼지도록 잡은 값이다 - 좁고 긴 골목형 구역에서 이 값이
    크면(예: 0.5) 사실상 모든 사람이 거의 같은 선 위로 몰려 화면상 "한 줄로
    줄지어 걷는" 것처럼 보인다(임의 튜닝값, 실측 보정 필요).
